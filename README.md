...
`reports/generation_run.jsonl`.

## Manual generation review

Ручная проверка проводится по четырём критериям: correctness, completeness, citation entailment
и качество языка/подачи. Каждый критерий получает 0–2 балла. Рубрика зафиксирована в
`data/eval/generation_review_rubric.md`, разметка — в
`data/eval/generation_human_labels.jsonl`, а воспроизводимая агрегация запускается командой:

```powershell
python scripts/summarize_generation_human_eval.py
```

Первая проверка дала 246/256 (96,09%), но strict pass по всем четырём критериям получили только
23 из 32 ответов (71,88%). Найдены пять неполных ответов, два citation gaps, два дефекта подачи и
одна подтверждённая retrieval-ошибка для `q010_ru`. Из десяти запросов без qrel в контексте девять
всё же дали полностью корректный ответ по альтернативным официальным фрагментам — текущие qrels
неполны и требуют отдельной adjudication.

Результаты находятся в `reports/generation_human_eval.*`. Это разметка одного зависимого
ревьюера, поэтому следующий этап перед публикацией benchmark-цифр — слепая вторая проверка и
разрешение расхождений между ревьюерами.
